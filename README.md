...
这是 GPT × Ripple 一起搓、早期版本经 Fable51 工程审校的会眨眼、会甩尾、会明显动耳、会闭眼趴睡、会缩成咕噜噜毛团，也会用四条独立小腿自然迈步来陪你的 `v0.19.1`。
...
扩展设置 → 糯叽桌宠 → **小气泡与陪伴**：选择场景，每行写一句，自动保存并随机使用；留空使用默认台词，也可恢复当前场景默认。支持 `{时间}`、`{时段}`、`{角色名}`、`{今日卡数}`、`{今日层数}`。每个场景最多 30 句、每句 240 字，按纯文本显示。

长按 600 ms 或点设置里的“听听今日播报”；键盘聚焦糯叽后 Shift + Enter / 空格也可播报。问候和播报使用设备当地时间，跨午夜自动换新一天，不发额外模型请求。

统计从启用后开始，只记录观察到生成过程的新角色回复；不回填以前的活动，不扫描其他聊天文件。用户消息、系统消息、重生成、续写、候选切换、旧聊天加载均不增加今日层数。卡数按当天有新回复的角色头像标识去重，群聊按实际回复角色计算。删除已计回复不扣除当天活动量。
...
- 大小范围为 **40%–150%**，已有大小设置保留。
- 移除当前聊天角色回复总数及其变量，播报只保留当地时间、今日卡数和今日新增层数，仍为 5 秒自动关闭。旧自定义台词中含旧楼层变量的句子会清理，原文保存在扩展设置的 `legacyFloorBubbles` 备份中。
- **日常陪伴**：主动气泡至少间隔 10 秒，每次最多显示 5 秒，等待回信时也不会常驻。
- **安静陪伴**：不主动冒泡；摸摸、蹭蹭、长按和设置预览仍可回应。动画与今日统计正常运行，“显示小气泡”总开关依然有效。
- 设置里的“台词用在哪里”可选 **所有角色通用 / 当前角色专属**。专属台词按场景覆盖，留空或恢复后继承通用台词；通用台词留空使用内置默认。预览按照正在编辑的范围展示。
- 同一角色的不同聊天共用专属台词；切换角色会同步刷新编辑器。按角色头像文件名关联，不受角色列表排序影响；替换为不同文件名的角色或重新导入卡后可能需要重新设置。群聊使用通用台词，不猜测归属。

验证：`node tools/test-companion.mjs` 与 `node tools/test-companion-controller.mjs`。包括范围继承、角色切换与编辑写入、旧台词备份、安静模式、气泡节流、5 秒消失和 40% 设置保留；手机酒馆视觉效果仍需真机验收。
