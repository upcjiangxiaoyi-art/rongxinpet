...
const legacyCode = await readFile(new URL('../legacy-compat.js',import.meta.url),'utf8');
const legacy = await import('data:text/javascript;base64,'+Buffer.from(legacyCode).toString('base64'));
...
api.petRongxin();assert.ok(classes.has('is-visible'),'quiet still answers manual petting');
...
ctx.extensionSettings.rongxin_pet={scale:40};assert.equal(api.getSettings().scale,40,'saved 40 percent survives reload');
ctx.extensionSettings.rongxin_pet.scale=20;assert.equal(api.getSettings().scale,40,'minimum is 40 percent');
...
elements['rongxin-bubble-scene'].value='chat';
elements['rongxin-bubble-scope'].value='general';elements['rongxin-bubble-scope'].options=[{},{}];
...
const scope=elements['rongxin-bubble-scope'],editor=elements['rongxin-bubble-lines'];
...
elements['rongxin-bubble-reset'].handlers.click();assert.equal(editor.value,'');assert.equal(cat.say('chat'),'通用');
