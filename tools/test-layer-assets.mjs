...
async function pixels(name){const im=await loadImage(new URL('nuoji-'+name+'.png',root).pathname);assert.equal(im.width,1185);assert.equal(im.height,1327);const c=createCanvas(1185,1327),ctx=c.getContext('2d');ctx.drawImage(im,0,0);return ctx.getImageData(0,0,1185,1327).data;}
