...
const {getNuzzlePose:pose,getNuzzlePoint:point,NUZZLE_DURATION_MS:duration,PET_STATES,RongxinRenderer,getWalkStrideLength}=mod;
for(let i=0;i<=420;i++){
    const p=pose(i/100);
...
RongxinRenderer.prototype.setState.call(renderer,'nuzzling');
...
const context=vm.createContext({window:win,document:{getElementById(){return null}},Date:{now:()=>clock},performance:{now:()=>clock},console,URL,PET_STATES,NUZZLE_DURATION_MS:duration,WAVE_DURATION_MS:mod.WAVE_DURATION_MS,RongxinRenderer,getWalkStrideLength,fakeRenderer,root});
let code=await readFile(new URL('../index.js',import.meta.url),'utf8');
code=code.slice(0,code.lastIndexOf("if (document.readyState === 'loading')"))
    .replace(/^import .*;$/gm, '').replaceAll('import.meta.url',JSON.stringify(url.href)).replaceAll('export function','function');
vm.runInContext(code+`\nrenderer=fakeRenderer; ui={root,bubble:{classList:{remove(){},add(){}}}}; settings={enabled:true,autoWalk:false,showBubble:false};
 globalThis.test={wave(){transitionTo(PET_STATES.WAVE,{duration:600,force:true})},doublePetRongxin,beginThinking,handleTypingInput,handlePointerMove,
...
app.doublePetRongxin();assert.equal(app.state(),'nuzzling');advance(4199);assert.equal(app.state(),'nuzzling');advance(1);assert.equal(app.state(),'idle');
app.doublePetRongxin();advance(1000);app.doublePetRongxin();advance(3200);assert.equal(app.state(),'nuzzling');advance(1000);assert.equal(app.state(),'idle');
app.doublePetRongxin();const nuzzleTimer=app.reaction();
...
app.doublePetRongxin();const beforeDrag=app.reaction();app.setDrag();
...
app.doublePetRongxin();const beforeGeneration=app.reaction();app.beginThinking('test');
